...
if (!class_exists('InPulse_footer_area')) {
    /**
     * Footer area
     *
     *
     * @class 		InPulse_footer_area
     * @version		1.0
     * @category	Class
     * @author 		WebGeniusLab
     */
    ...
    class InPulse_footer_area {
        ...
        */
        ...
        function __construct () {
            ...
            $footer_bg_color = InPulse_Theme_Helper::get_option('footer_bg_color');
            // copyright option
            $copyright_switch = InPulse_Theme_Helper::get_option('copyright_switch');

            //add global variables
            $this->footer_full_width = InPulse_Theme_Helper::get_option('footer_full_width');
            ...
            $style .= InPulse_Theme_Helper::bg_render('footer','mb_footer_switch','on');
            $style = !empty($style) ? ' style="'.esc_attr($style).'"' : '' ;


            //hide if 404 page
            $page_not_found = InPulse_Theme_Helper::get_option('404_show_footer');
            ...
            */
            if ($footer_switch || $copyright_switch) {
                echo "<footer class='footer clearfix'".$style." id='footer'>";
                    if ($footer_switch) {
                        $footer_content_type = InPulse_Theme_Helper::options_compare('footer_content_type','mb_footer_switch','on');
        ...
        private function get_wave_html(){
            $wave_switch = InPulse_Theme_Helper::options_compare('footer_add_wave','mb_footer_switch','on');
            if((bool) $wave_switch){

                $wave_height = InPulse_Theme_Helper::get_option('footer_wave_height');
                ...
                echo "<div class='inpulse_wave_footer' style='height:".(int) $wave_height."px;'>";
        ...
        private function get_footer_vars($optn_1 = null){
            ...
            //Get options
            $footer_spacing = InPulse_Theme_Helper::options_compare('footer_spacing','mb_footer_switch','on');
            $footer_border = InPulse_Theme_Helper::options_compare('footer_add_border','mb_footer_switch','on');
            $footer_border_color = InPulse_Theme_Helper::options_compare('footer_border_color','mb_footer_switch','on');

            // Only for widgets in footer
            if ($optn_1 == 'widgets') {
                $footer_options['widget_columns'] = InPulse_Theme_Helper::get_option('widget_columns');
                $footer_options['widget_columns_2'] = InPulse_Theme_Helper::get_option('widget_columns_2');
                $footer_options['widget_columns_3'] = InPulse_Theme_Helper::get_option('widget_columns_3');
                $footer_align = InPulse_Theme_Helper::get_option('footer_align');
        ...
        private function main_footer_get_page()
        {
            ...
            echo "<div class='row-footer'".$footer_style.">";
                ...
                $footer_page_select = InPulse_Theme_Helper::options_compare('footer_page_select', 'mb_footer_switch', 'on');
        ...
        {
            //Get options
            $copyright_spacing = InPulse_Theme_Helper::options_compare('copyright_spacing', 'mb_copyright_switch', 'on');
        ...
        {
            $bg_color = InPulse_Theme_Helper::options_compare('copyright_bg_color','mb_copyright_switch','on');
        ...
        {
            $editor = InPulse_Theme_Helper::get_option('copyright_editor');

            if ($this->mb_copyright_switch == 'on') {
                $editor = rwmb_meta('mb_copyright_editor');
            }
            ?>
            <div class='copyright'<?php echo InPulse_Theme_Helper::render_html($this->copyright_style()); ?> >
                <?php if (!$this->footer_full_width) echo "<div class='wgl-container'>"; ?>
                    <div class='row' <?php echo InPulse_Theme_Helper::render_html($this->copyright_spacing());?> >
