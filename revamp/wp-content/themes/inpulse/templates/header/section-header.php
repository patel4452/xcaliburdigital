...
if (!class_exists('InPulse_get_header')) {
	class InPulse_get_header{
	    ...
	    public function header_vars(){
	    	...
	    	$this->name_preset = InPulse_Theme_Helper::get_option('header_def_js_preset');
	    	$get_def_name = get_option( 'inpulse_set_preset' );
	    	if( !$this->in_array_r($this->name_preset, get_option( 'inpulse_set_preset' ))){    				
	    	...
	    	if (class_exists( 'RWMB_Loader' ) && $this->id !== 0) {
	    		...
	    		if (!empty($customize_header) && $customize_header != 'default') {
	    			if( !$this->in_array_r(rwmb_meta('mb_customize_header'), get_option( 'inpulse_set_preset' ))){    	
	    			}else{
	    				$get_def_name = get_option( 'inpulse_set_preset' );
		...
		public function init(){
	        ...
	        //hide if 404 page
	        $page_not_found = InPulse_Theme_Helper::get_option('404_show_header');
		...
		*/
	   	public function header_class(){
	   		$header_shadow = InPulse_Theme_Helper::get_option( 'header_shadow',$this->name_preset, $this->def_preset );
	   		$header_on_bg = InPulse_Theme_Helper::get_option( 'header_on_bg',$this->name_preset, $this->def_preset );
		...
		*/	    
		...
		public function header_bar_editor($location = null,$position = null){
			...
			*/
			...
			${'header_'.$position.'_editor'} = InPulse_Theme_Helper::get_option($location.'_header_bar_'.$position.'_editor', $this->name_preset, $this->def_preset);
		...
		*/	    
		...
		public function header_bar_delimiter($k = null){
			...
			$header_sticky_builder = InPulse_Theme_Helper::get_option('sticky_header');
				...
				*/
				...
				$height = InPulse_Theme_Helper::get_option( 'bottom_header_delimiter'.$get_number.'_height', $this->name_preset, $this->def_preset );	
				$width = InPulse_Theme_Helper::get_option( 'bottom_header_delimiter'.$get_number.'_width', $this->name_preset, $this->def_preset );	    

				$bg_color = InPulse_Theme_Helper::get_option( 'bottom_header_delimiter'.$get_number.'_bg', $this->name_preset, $this->def_preset );

				$margin = InPulse_Theme_Helper::get_option( 'bottom_header_delimiter'.$get_number.'_margin', $this->name_preset, $this->def_preset );
		        ...
		        if ( $this->html_render === 'sticky' ) {
		        	$custom_sticky = InPulse_Theme_Helper::get_option('bottom_header_delimiter'.$get_number.'_sticky_custom', $this->name_preset, $this->def_preset);
		        	if( !empty($custom_sticky) ){
		        		$bg_color = InPulse_Theme_Helper::get_option('bottom_header_delimiter'.$get_number.'_sticky_color', $this->name_preset, $this->def_preset);
		        		$height  = InPulse_Theme_Helper::get_option('bottom_header_delimiter'.$get_number.'_sticky_height', $this->name_preset, $this->def_preset);
		        	}
		        }
		...
		*/	    
		...
		public function header_bar_button($k = null){
			...
			*/
			...
			$get_number = (int) filter_var($k, FILTER_SANITIZE_NUMBER_INT);
			$button_text = InPulse_Theme_Helper::get_option('bottom_header_button'.$get_number.'_title', $this->name_preset, $this->def_preset);	

			$link = InPulse_Theme_Helper::get_option('bottom_header_button'.$get_number.'_link', $this->name_preset, $this->def_preset);	
			
			$target = InPulse_Theme_Helper::get_option('bottom_header_button'.$get_number.'_target', $this->name_preset, $this->def_preset);	

			$size = InPulse_Theme_Helper::get_option('bottom_header_button'.$get_number.'_size', $this->name_preset, $this->def_preset);	
			
			$options_btn = $this->html_render === 'sticky' ? '_sticky' : '';

			$customize = InPulse_Theme_Helper::get_option('bottom_header_button'.$get_number.'_custom'.$options_btn, $this->name_preset, $this->def_preset);	

			$customize = empty($customize) ? 'def' : 'color';

			$bg_color = InPulse_Theme_Helper::get_option('bottom_header_button'.$get_number.'_bg'.$options_btn, $this->name_preset, $this->def_preset);	

			$bg_color = isset($bg_color['rgba']) ? $bg_color['rgba'] : '';

			$text_color = InPulse_Theme_Helper::get_option('bottom_header_button'.$get_number.'_color_txt'.$options_btn, $this->name_preset, $this->def_preset);	

			$text_color = isset($text_color['rgba']) ? $text_color['rgba'] : '';

			$border_color = InPulse_Theme_Helper::get_option('bottom_header_button'.$get_number.'_border'.$options_btn, $this->name_preset, $this->def_preset);	
			$border_color = isset($border_color['rgba']) ? $border_color['rgba'] : '';

			$bg_color_hover = InPulse_Theme_Helper::get_option('bottom_header_button'.$get_number.'_hover_bg'.$options_btn, $this->name_preset, $this->def_preset);	
			$bg_color_hover = isset($bg_color_hover['rgba']) ? $bg_color_hover['rgba'] : '';

			$text_color_hover = InPulse_Theme_Helper::get_option('bottom_header_button'.$get_number.'_hover_color_txt'.$options_btn, $this->name_preset, $this->def_preset);	
			$text_color_hover = isset($text_color_hover['rgba']) ? $text_color_hover['rgba'] : '';

			$border_color_hover = InPulse_Theme_Helper::get_option('bottom_header_button'.$get_number.'_hover_border'.$options_btn, $this->name_preset, $this->def_preset);	
			$border_color_hover = isset($border_color_hover['rgba']) ? $border_color_hover['rgba'] : '';

			$border_radius = InPulse_Theme_Helper::get_option('bottom_header_button'.$get_number.'_radius', $this->name_preset, $this->def_preset);	
			switch ($size) {
				case 's':
					$size = 'sm';
					break;				
			...
			$button_css_id =  uniqid( "inpulse_button_" );
	        ...
	        // Register css
	        if (!empty($styles)) {
	        	InPulse_Theme_Helper::enqueue_css($styles);
	        }
    		...
    		echo '<div class="header_button">';
    			echo '<div class="wrapper">';
		            ...
		            ?>
		            <div <?php echo InPulse_Theme_Helper::render_html($this->get_render_attribute_string( 'wrapper' )); ?>>
		                <a <?php echo InPulse_Theme_Helper::render_html($this->get_render_attribute_string( 'button' )); ?>>
        ...
        public function render_text($settings) {
            ...
            ?>
            <span <?php echo InPulse_Theme_Helper::render_html($this->get_render_attribute_string( 'content-wrapper' )); ?>>
                <span <?php echo InPulse_Theme_Helper::render_html($this->get_render_attribute_string( 'text' )); ?>><?php echo InPulse_Theme_Helper::render_html($settings['text']); ?></span>
            </span>
            <?php
        }
		...
		public function get_render_attribute_string( $element ) {
			...
			return InPulse_Theme_Helper::render_html_attributes( $this->render_attributes[ $element ] );
		}
		...
		*/	    
		...
		public function header_bar_spacer($location = null, $key = null){
			...
			*/
			...
			$spacer = InPulse_Theme_Helper::get_option($location.'_header_spacer'.$get_number, $this->name_preset, $this->def_preset);	    	
		...
		*/
	   	public function build_header_layout($section = 'bottom'){
	   		...
	   		$header_layout = InPulse_Theme_Helper::get_option($section.'_header_layout', $this->name_preset, $this->def_preset);
	   		$lavalamp_active = InPulse_Theme_Helper::get_option('lavalamp_active', $this->name_preset, $this->def_preset);
			...
			*/
			...
			switch ($section) {
				case 'bottom':
					$width_container = InPulse_Theme_Helper::get_option('header'.$s.'_full_width',$this->name_preset, $this->def_preset);
				...
				case 'sticky':
					$width_container = InPulse_Theme_Helper::get_option('header_custom_sticky_full_width');
					if ($width_container == '1') {
			   			$class = "fullwidth-wrapper";
			   		} else {
			   			$class = 'wgl-container';
			...
			* @access private
			*/
			$header_background = InPulse_Theme_Helper::get_option('header'.$s.'_background', $this->name_preset, $this->def_preset);
			$header_background_image = InPulse_Theme_Helper::get_option('header'.$s.'_background_image', $this->name_preset, $this->def_preset);
			$header_background_image = isset($header_background_image['url']) ? $header_background_image['url'] : '';  

			$header_color = InPulse_Theme_Helper::get_option('header'.$s.'_color', $this->name_preset, $this->def_preset);
			$header_bottom_border = InPulse_Theme_Helper::get_option('header'.$s.'_bottom_border', $this->name_preset, $this->def_preset);
			$header_border_height = InPulse_Theme_Helper::get_option('header'.$s.'_border_height', $this->name_preset, $this->def_preset);
			$header_border_height = $header_border_height['height'] ?? '';
			$header_bottom_border_color = InPulse_Theme_Helper::get_option('header'.$s.'_bottom_border_color', $this->name_preset, $this->def_preset);
			...
			*/
			...
			$header_sticky_height = InPulse_Theme_Helper::get_option('header_sticky_height');
	   		...
	   		$header_height = InPulse_Theme_Helper::get_option('header'.$s.'_height', $this->name_preset, $this->def_preset);
	        ...
	        switch ($this->html_render) {
	        	case 'sticky':
		        	if (!empty($header_sticky_height["height"])) {
			        	...
			        	if(!empty($width)){
			        		$style .= 'width:'.(int)$header_sticky_height["height"].'px;';
			        	}
			        	$style = ' style="'.$style.'"';
			        }
	        		break;	        	
	        	...
	        	case 'mobile':
		        	if (!empty($header_mobile_height["height"])) {
			        	...
			        	if(!empty($width)){
			        		$style .= 'width:'.(int)$header_mobile_height["height"].'px;';
			        	}
			        	$style = ' style="'.$style.'"';
	   	...
	   	private function side_panel_style_icon($s = '_middle', $section){   		
	   		...
	   		$sticky_icon_switcher = InPulse_Theme_Helper::get_option('bottom_header_side_panel_sticky_custom', $this->name_preset, $this->def_preset);	
			...
			$icon_background = InPulse_Theme_Helper::get_option('bottom_header_side_panel'.$value.'_background', $this->name_preset, $this->def_preset);
			$icon_color = InPulse_Theme_Helper::get_option('bottom_header_side_panel'.$value.'_color', $this->name_preset, $this->def_preset);
			...
			* @access private
			*/
			$dispay = InPulse_Theme_Helper::get_option('header_column'.$s.'_'.$area.'_display', $this->name_preset, $this->def_preset);
			$v_align = InPulse_Theme_Helper::get_option('header_column'.$s.'_'.$area.'_vert', $this->name_preset, $this->def_preset);
			$h_align = InPulse_Theme_Helper::get_option('header_column'.$s.'_'.$area.'_horz', $this->name_preset, $this->def_preset);
		...
		*/
	   	public function build_header_mobile_menu($preset = null, $def_preset = null){
	   		...
	   		$header_queris = InPulse_Theme_Helper::get_option('header_mobile_queris' ,$preset, $def_preset);
	   		$sub_menu_position = InPulse_Theme_Helper::get_option('mobile_position');
	   		$mobile_header_custom =  InPulse_Theme_Helper::get_option('mobile_header');
		...
		*/
        public static function get_logo($location, $menu = false){
            ...
            $header_logo_src = InPulse_Theme_Helper::get_option('header_logo');
            ...
            // Get Sticky Logotype
            $logo_sticky_src =  InPulse_Theme_Helper::get_option('logo_sticky');
            ...
            $logo_mobile_src =  InPulse_Theme_Helper::get_option('logo_mobile'.$menu);
            ...
            $logo_height_custom = InPulse_Theme_Helper::get_option('logo_height_custom');
            $logo_height = InPulse_Theme_Helper::get_option('logo_height');
            $logo_height = $logo_height['height'] ?? '';
            

            $sticky_logo_height_custom = InPulse_Theme_Helper::get_option('sticky_logo_height_custom');
            $sticky_logo_height = InPulse_Theme_Helper::get_option('sticky_logo_height');
            $sticky_logo_height = $sticky_logo_height['height'] ?? '';

            $mobile_logo_height_custom = InPulse_Theme_Helper::get_option('mobile_logo'.$menu.'_height_custom');
            $mobile_logo_height = InPulse_Theme_Helper::get_option('mobile_logo'.$menu.'_height');
			...
			switch (true) {
				case $location == 'bottom':
					if (!empty($header_logo_src)) {
						?>
						<img class="default_logo" src="<?php echo esc_url($header_logo_src); ?>" alt="<?php echo esc_attr($def_img_alt); ?>" <?php echo InPulse_Theme_Helper::render_html($logo_height_style);?>>
					<?php
					} else {
						?>
						<h1 class="logo-name">
				...
				case !empty($logo_sticky_src) && $location == 'sticky':
					?>
					<img class="logo-sticky" src="<?php echo esc_url($logo_sticky_src);?>" alt="<?php echo esc_attr($sticky_img_alt); ?>" <?php echo InPulse_Theme_Helper::render_html($sticky_height_style);?>>
					<?php
					break;

				case !empty($logo_mobile_src) && $location == 'mobile':
					?>
					<img class="logo-mobile" src="<?php echo esc_url($logo_mobile_src);?>" alt="<?php echo esc_attr($mobile_img_alt); ?>" <?php echo InPulse_Theme_Helper::render_html($mobile_height_style);?>>
					<?php
					break;
		...
		*/
	    public function search($html_render = '',$location, $section){
	    	$description = esc_html__('Type To Search', 'inpulse');
			$search_style = InPulse_Theme_Helper::get_option('search_style');
	        ...
	        $search_class = ' search_'.InPulse_Theme_Helper::get_option('search_style');

	        $border_width = InPulse_Theme_Helper::get_option( 'bottom_header_item_search_border_width' , $this->name_preset, $this->def_preset);
	        $border_width = $border_width['height'] ?? '';

			$options_btn = $this->html_render === 'sticky' ? '_sticky' : '';

			$customize = InPulse_Theme_Helper::get_option('bottom_header_item_search_custom'.$options_btn, $this->name_preset, $this->def_preset);	

			$customize = empty($customize) ? 'def' : 'color';

			$bg_color = InPulse_Theme_Helper::get_option('bottom_header_item_search_bg'.$options_btn, $this->name_preset, $this->def_preset);	

			$bg_color = isset($bg_color['rgba']) ? $bg_color['rgba'] : '';

			$text_color = InPulse_Theme_Helper::get_option('bottom_header_item_search_color_txt'.$options_btn, $this->name_preset, $this->def_preset);	

			$text_color = isset($text_color['rgba']) ? $text_color['rgba'] : '';

			$border_color = InPulse_Theme_Helper::get_option('bottom_header_item_search_border'.$options_btn, $this->name_preset, $this->def_preset);	
			$border_color = isset($border_color['rgba']) ? $border_color['rgba'] : '';

			$bg_color_hover = InPulse_Theme_Helper::get_option('bottom_header_item_search_hover_bg'.$options_btn, $this->name_preset, $this->def_preset);	
			$bg_color_hover = isset($bg_color_hover['rgba']) ? $bg_color_hover['rgba'] : '';

			$text_color_hover = InPulse_Theme_Helper::get_option('bottom_header_item_search_hover_color_txt'.$options_btn, $this->name_preset, $this->def_preset);	
			$text_color_hover = isset($text_color_hover['rgba']) ? $text_color_hover['rgba'] : '';

			$border_color_hover = InPulse_Theme_Helper::get_option('bottom_header_item_search_hover_border'.$options_btn, $this->name_preset, $this->def_preset);	
			$border_color_hover = isset($border_color_hover['rgba']) ? $border_color_hover['rgba'] : '';


			$search_css_id =  uniqid( "inpulse_search_" );
		        ...
		        ob_start();
					...
					if ($customize != 'def') {
						if ($customize == 'color') {
							...
							echo "#$search_css_id:hover {
									  border-color: ".(!empty($border_color_hover) ? esc_attr($border_color_hover) : 'transparent')." !important;
    		...
    		if($this->html_render !== 'mobile'){
	    		if(!empty($border_width)){
	    			$this->add_render_attribute( 'search', 'style', ['border-style: solid; border-width: '.esc_attr( (int) $border_width) .'px;' ]);
	    		}

	    		if ($customize != 'def') {
					if ($customize == 'color') {	
	    				$this->add_render_attribute( 'search', 'style', [ 'color: '.(!empty($text_color) ? esc_attr($text_color) : 'transparent').';', 'border-color:' .(!empty($border_color) ? esc_attr($border_color) : 'transparent').';', 'background-color:'.esc_attr($bg_color).';' ] );
					}
				}
    		}    		
	        ...
	        $output = '<div class="header_search'.esc_attr($search_class).'"'.( $this->row_style_height($location, $section, $this->html_render !== 'mobile' ? true : false) ).'>';
				
				$output .= '<div '.(InPulse_Theme_Helper::render_html($this->get_render_attribute_string( 'search' ))).'>';
					...
					$output .= '<div class="header_search-field">';
						if ($search_style === 'alt') {
							$output .= '<div class="header_search-wrap">';
								$output .= '<div class="inpulse_module_double_headings aleft">';
								$output .= '<h3 class="header_search-heading_description heading_title">'.apply_filters('inpulse_desc_search', $description) .'</h3>';
								$output .= '</div>';
							$output .= '</div>';
							$output .= '<div class="header_search-close"></div>';
						}else{
							if($this->html_render !== 'mobile'){
								$output .= '<div class="header_search-button-wrapper"'.$this->row_style_height($location, $section, true).'>';
									$output .= '<div class="header_search-button"></div>';
								$output .= '</div>';	
							}
	    ...
	    public function login_in($location, $section){
	        ...
	        echo "<div class='login-in woocommerce'".$this->row_style_height($location, $section).">";
	        	...
	        	if ( is_user_logged_in() ){
	        		...
	        		echo "<a class='login-in_link-logout' href='".esc_url( $link_logout )."'>".esc_html__('Logout', 'inpulse')."</a>";
	        	}else{
	        		echo "<a class='login-in_link' href='".esc_url_raw( $url )."'>".esc_html__('Login', 'inpulse')."</a>";
	        	}
		...
		*/
	    public function cart($location, $section){
	    	$output = '';
	        echo "<div class='mini-cart woocommerce'".( $this->row_style_height($location, $section, $this->html_render !== 'mobile' ? true : false) ).">".$this->icon_cart().self::woo_cart()."</div>";
	    }	   
		...
		public function icon_cart() {
	        ...
	        $border_width = InPulse_Theme_Helper::get_option( 'bottom_header_cart_border_width' , $this->name_preset, $this->def_preset);
	        $border_width = $border_width['height'] ?? '';

			$options_btn = $this->html_render === 'sticky' ? '_sticky' : '';

			$customize = InPulse_Theme_Helper::get_option('bottom_header_cart_custom'.$options_btn, $this->name_preset, $this->def_preset);	

			$customize = empty($customize) ? 'def' : 'color';

			$bg_color = InPulse_Theme_Helper::get_option('bottom_header_cart_bg'.$options_btn, $this->name_preset, $this->def_preset);	

			$bg_color = isset($bg_color['rgba']) ? $bg_color['rgba'] : '';

			$text_color = InPulse_Theme_Helper::get_option('bottom_header_cart_color_txt'.$options_btn, $this->name_preset, $this->def_preset);	

			$text_color = isset($text_color['rgba']) ? $text_color['rgba'] : '';

			$border_color = InPulse_Theme_Helper::get_option('bottom_header_cart_border'.$options_btn, $this->name_preset, $this->def_preset);	
			$border_color = isset($border_color['rgba']) ? $border_color['rgba'] : '';

			$bg_color_hover = InPulse_Theme_Helper::get_option('bottom_header_cart_hover_bg'.$options_btn, $this->name_preset, $this->def_preset);	
			$bg_color_hover = isset($bg_color_hover['rgba']) ? $bg_color_hover['rgba'] : '';

			$text_color_hover = InPulse_Theme_Helper::get_option('bottom_header_cart_hover_color_txt'.$options_btn, $this->name_preset, $this->def_preset);	
			$text_color_hover = isset($text_color_hover['rgba']) ? $text_color_hover['rgba'] : '';

			$border_color_hover = InPulse_Theme_Helper::get_option('bottom_header_cart_hover_border'.$options_btn, $this->name_preset, $this->def_preset);	
			$border_color_hover = isset($border_color_hover['rgba']) ? $border_color_hover['rgba'] : '';


			$cart_css_id =  uniqid( "inpulse_woo_" );
    		...
    		$this->add_render_attribute( 'cart', 'title', esc_attr__( 'Click to open Shopping Cart', 'inpulse' ) );

    		if($this->html_render !== 'mobile'){
	    		if(!empty($border_width)){
	    			$this->add_render_attribute( 'cart', 'style', ['border-style: solid; border-width: '.esc_attr( (int) $border_width) .'px;' ]);
	    		}

	    		if ($customize != 'def') {
					if ($customize == 'color') {	
	    				$this->add_render_attribute( 'cart', 'style', [ 'color: '.(!empty($text_color) ? esc_attr($text_color) : 'transparent').';', 'border-color:' .(!empty($border_color) ? esc_attr($border_color) : 'transparent').';', 'background-color:'.esc_attr($bg_color).';' ] );
					}
				}
    		}    		
			...
			?>
				<a <?php echo InPulse_Theme_Helper::render_html( $this->get_render_attribute_string( 'cart' ) ) ;?> >
		...
		public static function woo_cart(){
			$description = esc_html__('Cart', 'inpulse');
			
			ob_start();
				echo '<div class="wgl-woo_mini_cart">';
					echo '<span class="header_cart-heading_description">'.apply_filters('inpulse_heading_cart', $description) .'</span>';
