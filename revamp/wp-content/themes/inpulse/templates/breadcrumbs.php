...
*
*
* @class 		InPulse_breadcrumbs
* @version		1.0
* @category	Class
* @author 		WebGeniusLab
*/
if (!class_exists('InPulse_breadcrumbs')) {
    class InPulse_breadcrumbs {
		...
		private function text_options(){
			$this->text_home     = esc_html__('Home', 'inpulse'); // text for the 'Home' link
			$this->text_category = esc_html__('Archive by Category "%s"', 'inpulse'); // text for a category page
			$this->text_search   = esc_html__('Search Results for "%s" Query', 'inpulse'); // text for a search results page
			$this->text_tag      = esc_html__('Posts Tagged "%s"', 'inpulse'); // text for a tag page
			$this->text_author   = esc_html__('Articles Posted by %s', 'inpulse'); // text for an author page
			$this->text_404      = esc_html__('Error 404', 'inpulse'); // text for the 404 page
			$this->text_page     = esc_html__('Page %s', 'inpulse'); // text 'Page N'
			$this->text_cpage    = esc_html__('Comment Page %s', 'inpulse'); // text 'Comment Page N'		
		}
			...
			if (is_home() || is_front_page()) {
				...
				elseif (function_exists('is_product') && is_product()) {
	   				if(function_exists('inpulse_woocommerce_breadcrumb')){
	   					...
	   					$args['delimiter'] = $sep;
	   					if ($show_home_link) $out .= $sep;
	   					$out .= inpulse_woocommerce_breadcrumb($args);
	   				}
	   			}
				$out .= $wrap_after;

				echo InPulse_Theme_Helper::render_html($out);
			}
    	}
    }
    new InPulse_breadcrumbs();
}