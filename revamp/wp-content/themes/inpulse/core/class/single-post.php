...
/**
* InPulse Theme Helper
*
*
* @class        InPulse_Theme_Helper
* @version      1.0
* @category Class
* @author       WebGeniusLab
*/

if (!class_exists('InPulse_Single_Post')) {

	class InPulse_SinglePost{
		/**
	     * @var SinglePost
	     */
	    ...
	    public function render_bg ( $link_feature = false, $image_size = 'full',$aq_image = false, $data_animation = null, $show_media = true ) {
			...
			$featured_image = InPulse_Theme_Helper::options_compare('featured_image_type', 'mb_featured_image_conditional', 'custom');	
			if ($featured_image == 'replace') {
				$featured_image_replace = InPulse_Theme_Helper::options_compare('featured_image_replace', 'mb_featured_image_conditional', 'custom');	
			}
	    	...
	    	if((bool) $media && (bool) $show_media){
	    		if($this->post_format === 'video' && $video_style == 'bg_video'){
	    			echo InPulse_Theme_Helper::render_html($media);
			...
			<?php
           	...
           	?>
           		<div class="blog-post_quote-author">
           			<?php
           			echo InPulse_Theme_Helper::render_html($autor_avatar);
	    ...
	    public function image_size_render($image_size){
	    	$arr = array();
	    	switch ($image_size) {
	    		case 'inpulse-840-620':
	    		$arr['width'] = '840';
	    		$arr['height'] = '620';
	    		break;	    		
	    		case 'inpulse-740-560':
	    		$arr['width'] = '740';
	    		$arr['height'] = '560';
	    		break;	 	    		
	    		case 'inpulse-700-550':
	    		$arr['width'] = '700';
	    		$arr['height'] = '550';
	    		break;	    		
	    		case 'inpulse-440-440':
	    		$arr['width'] = '440';
	    		$arr['height'] = '440';
	    		break;	    		
	    		case 'inpulse-420-300':
	    		$arr['width'] = '420';
	    		$arr['height'] = '300';
	    		break;		    		
	    		case 'inpulse-180-180':
	    		$arr['width'] = '180';
	    		$arr['height'] = '180';
	    		break;	
	    		default:
	    ...
	    public function render_post_cats () {
			...
			if ($categories) {
				$post_categ = '';
				echo '<span class="in_cat">'.esc_html__('In:', 'inpulse').'</span>';
	    ...
	    public function render_excerpt ($symbol_count = false, $shortcode = false, $read_more = false, $read_more_text = false) {
			...
			if ((bool)InPulse_Theme_Helper::get_option('blog_post_listing_content') || $shortcode) {
				$post_content = preg_replace( '~\[[^\]]+\]~', '', $post_content);
				$post_content_stripe_tags = strip_tags($post_content);
				$output = InPulse_Theme_Helper::modifier_character($post_content_stripe_tags, $symbol_count, "...");
			} else {
				$output = $post_content;
			}
	    ...
	    public function get_post_views($postID){
		    ...
		    if($count==''){
		    	...
		    	echo '<span class="counts_text">'. esc_html__(' View', 'inpulse'). '</span>';
		    }else{
				echo '<span class="counts">'. esc_html($count). '</span>';
		    	echo '<span class="counts_text">'. esc_html__(' Views', 'inpulse'). '</span>';
		    }
			echo '</div>';
	    ...
	    public function render_author_info () {
			...
			$name_html = !empty($user_first) || !empty($user_last) ? '<h5 class="author-info_name">'.'<span class="author-excerpt_name">'.esc_html__('About', 'inpulse').'</span>'.$user_first.' '.$user_last.'</h5>' : '';
